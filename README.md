...
```
gas_provePossession(N) = 160.156M + 7.703M × log₂(N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)
```
...
| Metric | Value |
|--------|-------|
| Training datasets | 971 (updated 2026-05-29) |
