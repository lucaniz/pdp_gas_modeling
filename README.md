...
```
gas_provePossession(N) = 160.243M + 7.693M × log₂(N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)
```
...
| Metric | Value |
|--------|-------|
| Training datasets | 974 (updated 2026-05-31) |
