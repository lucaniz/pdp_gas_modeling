...
```
gas_provePossession(N) = 160.128M + 7.699M × log₂(N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)
```
...
| Metric | Value |
|--------|-------|
| Training datasets | 965 (updated 2026-05-28) |
