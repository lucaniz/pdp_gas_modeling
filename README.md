...
```
gas_provePossession(N) = 160.041M + 7.726M × log₂(N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)
```
...
| Metric | Value |
|--------|-------|
| Training datasets | 952 (updated 2026-05-25) |
