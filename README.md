...
```
gas_provePossession(N) = 160.173M + 7.695M × log₂(N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)N)
```
...
| Metric | Value |
|--------|-------|
| Training datasets | 973 (updated 2026-05-30) |
